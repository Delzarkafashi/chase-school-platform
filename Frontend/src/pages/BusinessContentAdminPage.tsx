...
type NewBusinessContent = Omit<BusinessContent, "id">;

const emptySection: NewBusinessContent = {
  section: "",
  eyebrow: "",
  title: "",
  text: "",
...
function BusinessContentAdminPage() {
  ...
  const [newSection, setNewSection] =
    useState<NewBusinessContent>(emptySection);

  const [sectionToDelete, setSectionToDelete] =
    useState<BusinessContent | null>(null);
  ...
  function updateNewField(
    field: keyof NewBusinessContent,
    value: string
  ) {
  ...
  async function handleSave(section: BusinessContent) {
    try {
      setSavingId(section.id);
      setSavedId(null);
      setError("");

      const updatedSection = await updateBusinessContent(
        section.id,
        section
      );
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const createdSection = await createBusinessContent({
  ...
  async function handleDelete() {
    ...
    try {
      ...
      await deleteBusinessContent(sectionToDelete.id);
        ...
        <div className="content-admin-page-heading">
          <div>
            <span className="dashboard-eyebrow">
              INNEHÅLL / FÖR FÖRETAG
            </span>

            <h1>Redigera För företag</h1>
            ...
            <div className="content-admin-form">
              ...
              <label>
                ...
                <input
                  ...
                  placeholder="Exempel: SAMARBETA MED OSS"
                />
              </label>
...
export default BusinessContentAdminPage;